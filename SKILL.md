...
笔记存放在 `notes/` 目录（路径由 `.env` 中的 `LOCAL_RAG_WORK_DIR` 决定，默认为 `~/.local/share/local-rag/notes`）。

- **创建笔记**: 直接用 Write 工具写入 `notes/<title>.md`，然后用 `create_note` MCP 工具同步到 RAG
...
1. **检查依赖**：`bash src/local_rag/scripts/install.sh`
2. **配置环境变量**：复制 `src/local_rag/.env.example` 为 `.env`，至少设置 `LLM_API_KEY`
3. **启动服务**：`bash src/local_rag/scripts/start.sh`（自动创建 venv、安装依赖、启动 Neo4j 和 daemon）
4. **模型下载**：BGE-M3 嵌入模型首次启动时自动从 ModelScope 下载（约 2-3 GB）
